#!/usr/bin/env python
""" A modification of the dev perform trade strategy, using CCXT to determine if the asset is
    trending, and then deciding to place a trade or not based on this.
"""
import logging
import math
import pandas as pd
...
class TrendTradeStrategy(StrategyPyBase):
    ...
    def format_status(self) -> str:
        ...
        lines.extend(["Mean: {:4.2f}".format(self._mean), "Trend is {}".format(str(self._trend))])
    ...
    def get_trend(self):
        """ Get the last hour of 5 minute data. """
        exchange = ccxt.ftx()
        coin = "BTC/USD"
        timeframe = '5m'
        tsi = exchange.fetch_ohlcv(coin, timeframe, limit=20)
        tspd = pd.DataFrame(tsi, columns=['time', 'open', 'high', 'low', 'close', 'volume'])
        self._mean = tspd['close'].mean()
        self._trend = tspd['close'].iloc[-1] > self._mean

...
def trading_pair_prompt():
    exchange = trend_trade_config_map.get("exchange").value
...
async def validate_order_amount(value: str) -> Optional[str]:
    try:
        exchange = trend_trade_config_map["exchange"].value
        trading_pair = trend_trade_config_map["trading_pair"].value

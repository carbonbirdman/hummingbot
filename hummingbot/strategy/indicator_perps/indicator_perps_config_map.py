...
def maker_trading_pair_prompt():
    derivative = indicator_perps_config_map.get("derivative").value
...
async def order_amount_prompt() -> str:
    derivative = indicator_perps_config_map["derivative"].value
    trading_pair = indicator_perps_config_map["market"].value
...
def on_validate_price_source(value: str):
    if value != "external_market":
        indicator_perps_config_map["price_source_derivative"].value = None
        indicator_perps_config_map["price_source_market"].value = None
        indicator_perps_config_map["take_if_crossed"].value = None
    if value != "custom_api":
        indicator_perps_config_map["price_source_custom_api"].value = None
    else:
        indicator_perps_config_map["price_type"].value = None


def price_source_market_prompt() -> str:
    external_market = indicator_perps_config_map.get("price_source_derivative").value
    return f'Enter the token trading pair on {external_market} >>> '


def validate_price_source_derivative(value: str) -> Optional[str]:
    if value == indicator_perps_config_map.get("derivative").value:
...
def validate_price_source_market(value: str) -> Optional[str]:
    market = indicator_perps_config_map.get("price_source_derivative").value
...
def validate_take_if_crossed(value: str) -> Optional[str]:
    ...
    if err_msg is not None:
        return err_msg
    price_source_enabled = indicator_perps_config_map["price_source_enabled"].value
    take_if_crossed = parse_cvar_value(indicator_perps_config_map["take_if_crossed"], value)
...
indicator_perps_config_map = {
    "strategy":
        ConfigVar(key="strategy",
                  prompt=None,
                  default="indicator_perps"),
    "derivative":
    ...
    "long_profit_taking_spread":
        ConfigVar(key="long_profit_taking_spread",
                  ...
                  required_if=lambda: indicator_perps_config_map.get("position_management").value == "Profit_taking",
                  type_str="decimal",
                  default=Decimal("0"),
    ...
    "ts_activation_spread":
        ConfigVar(key="ts_activation_spread",
                  ...
                  required_if=lambda: indicator_perps_config_map.get("position_management").value == "Trailing_stop",
                  type_str="decimal",
                  default=Decimal("0"),
                  validator=lambda v: validate_decimal(v, 0, 100, True),
                  prompt_on_new=True),
    ...
    "order_level_amount":
        ConfigVar(key="order_level_amount",
                  ...
                  required_if=lambda: indicator_perps_config_map.get("order_levels").value > 1,
                  type_str="decimal",
    ...
    "hanging_orders_cancel_pct":
        ConfigVar(key="hanging_orders_cancel_pct",
                  prompt="At what spread percentage (from mid price) will hanging orders be canceled? "
                         "(Enter 1 to indicate 1%) >>> ",
                  required_if=lambda: indicator_perps_config_map.get("hanging_orders_enabled").value,
                  type_str="decimal",
                  default=Decimal("10"),
                  validator=lambda v: validate_decimal(v, 0, 100, inclusive=False)),
    ...
    "ask_order_optimization_depth":
        ConfigVar(key="ask_order_optimization_depth",
                  ...
                  required_if=lambda: indicator_perps_config_map.get("order_optimization_enabled").value,
                  type_str="decimal",
    ...
    "price_type":
        ConfigVar(key="price_type",
                  ...
                  type_str="str",
                  required_if=lambda: indicator_perps_config_map.get("price_source").value != "custom_api",
                  default="mid_price",
    ...
    "price_source_derivative":
        ConfigVar(key="price_source_derivative",
                  ...
                  required_if=lambda: indicator_perps_config_map.get("price_source").value == "external_market",
                  type_str="str",
    ...
    "take_if_crossed":
        ConfigVar(key="take_if_crossed",
                  ...
                  required_if=lambda: indicator_perps_config_map.get(
